...
def build_extraction_prompt(catalogs: dict[str, list[str]]) -> str:
    ...
    return (
        ...
        "【基本原则】优先提取有明确证据的内容，并结合客单上下文、行业常见表达和字段关系进行合理推测与补全。"
        "能够合理判断的字段应尽量填写；仍无法判断时缺失标量=null，缺失多选=[]。"
        "原文明写0时才输出0；空白、待报价、未报价均为null。日期统一YYYY-MM-DD，可结合客单上下文合理补全年份；仍无法判断完整日期时为null并将原文写入global_note。"
        ...
        "【字段规则】"
        "order_type优先使用原文明写且匹配允许值的内容；未明写时可结合客单用途和上下文从允许值中合理判断，无法判断时为null。"
        "product_name只写产品品类，如徽章、纪念币、双面币、钥匙扣；不得包含客户、图案、尺寸、材质、颜色或用途，最多8个字符；无法判断时null。"
        "delivery_date识别交货日期，可结合上下文补全年份，但不得把下单日期当成交货日期。quantity识别订购数量，不得使用备品、包装或装箱数量。"
