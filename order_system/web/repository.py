    ...
    def create_workshop_records(
        ...
        for row in rows:
            ...
            if department_key in process_options:
                ...
                if len(employee_names) != 1 or employee_names[0] not in allowed_options:
                    raise ValueError(f"\u8ba2\u5355 {order_no} \u8bf7\u9009\u62e9{department_name}\u5de5\u827a")
            if department_key == "uv":
                employee_names = ["UV"]
            ...
            if department_key in tooling_departments:
                ...
                size_text = ""
                spec = ""
                if department_key not in {"polishing", "uv"}:
                    note_text = ""
                record_type = "rework" if department_key == "uv" and record_type == "rework" else "normal"
            clean_rows.append({
                "order_no": order_no,
                "unit_price": unit_price,
                "quantity": quantity,
    ...
    def outsource_receipt(self, record_ids: list[int]) -> dict[str, Any] | None:
        ...
        with self.connect() as conn:
            for chunk in self._id_chunks(ids):
                placeholders = ", ".join("?" for _ in chunk)
                rows.extend(conn.execute(
                    f"""SELECT id, order_no, process_name, factory_name, quantity, unit_price, amount, outsource_date
